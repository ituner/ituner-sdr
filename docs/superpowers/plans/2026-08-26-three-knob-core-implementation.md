...
def test_main_context_has_sidebar_actions_in_reading_order(self):
    ...
    self.assertEqual(tuple(control.control_id for control in context.controls), (
        "local_rx", "receivers", "audio", "digital", "dual", "settings",
    ))

def test_tune_command_quantizes_and_clamps(self):
    result = ui.knob_tune_frequency(1000.0, clicks=3, multiplier=1, step_hz=100)
    self.assertAlmostEqual(result, 1000.3)
```

- [x] **Step 2: Run focused UI tests and verify failure**

Run: `PYGAME_HIDE_SUPPORT_PROMPT=1 UI/.venv/bin/python -m unittest UI.test_knob_ui -v`
...
Run: `PYGAME_HIDE_SUPPORT_PROMPT=1 UI/.venv/bin/python -m unittest UI.test_knob_input UI.test_knob_controller UI.test_knob_ui -v`
...
Run: `python3 -m py_compile UI/knob_input.py UI/knob_controller.py UI/kiwi_gl_display.py UI/test_knob_input.py UI/test_knob_controller.py UI/test_knob_ui.py`
...
- Focused knob suite and complete UI discovery: 35 tests passed on the upstream-main branch.
- `py_compile` passed for the production and test modules changed by this slice.
- `git diff --check` passed.
- A final real macOS OpenGL launch with `--desktop --desktop-knobs --no-audio --no-remember-receiver --duration 3` rendered 69 frames at 22.7 fps and exited cleanly.
